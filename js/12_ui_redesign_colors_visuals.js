...
function v19HideAllPanels() {
  document.body.classList.add('v19-map-only');
  document.querySelectorAll('#ui .menuPanel').forEach(p => p.classList.remove('active'));
  document.querySelectorAll('#ui .menuTabs button').forEach(b => b.classList.remove('active'));
  document.getElementById('v19MapOnlyBtn')?.classList.add('active');
}

function v19ShowPanelMode() {
  document.body.classList.remove('v19-map-only');
}
...
function v18OrganizeTopUI() {
  ...
  if (menuTabs) {
    if (!document.getElementById('v19MapOnlyBtn')) {
      const mapBtn = document.createElement('button');
      mapBtn.id = 'v19MapOnlyBtn';
      mapBtn.textContent = 'Map';
      mapBtn.title = 'Hide panels and return to the map';
      mapBtn.addEventListener('click', v19HideAllPanels);
      menuTabs.insertBefore(mapBtn, menuTabs.firstChild);
    }
    menuTabs.querySelectorAll('button:not(#v19MapOnlyBtn)').forEach(btn => {
      btn.addEventListener('click', v19ShowPanelMode, true);
    });
    inner.insertBefore(menuTabs, topBar.nextSibling);
  }

  document.querySelectorAll('#ui .menuPanel').forEach(panel => {
    if (!panel.querySelector('.v19PanelClose')) {
      const close = document.createElement('button');
      close.className = 'v19PanelClose';
      close.textContent = 'Hide panel';
      close.addEventListener('click', v19HideAllPanels);
      panel.insertBefore(close, panel.firstChild);
    }
  });

  // Start in map-first mode so nothing large is in the player's face.
  v19HideAllPanels();
}
...
const V19_ORIGINAL_SET_ACTIVE_MENU = typeof setActiveMenu === 'function' ? setActiveMenu : null;
if (V19_ORIGINAL_SET_ACTIVE_MENU) {
  setActiveMenu = function setActiveMenuV19(panel) {
    v19ShowPanelMode();
    document.getElementById('v19MapOnlyBtn')?.classList.remove('active');
    return V19_ORIGINAL_SET_ACTIVE_MENU(panel);
  };
}
